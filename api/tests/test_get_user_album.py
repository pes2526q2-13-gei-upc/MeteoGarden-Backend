...
class GetUserAlbumTests(TestCase):
    def setUp(self):
        ...
        self.image1 = Image.objects.create(
            plant=self.plant1, url=file1, uploader=self.user
        )
        self.image2 = Image.objects.create(
            plant=self.plant2, url=file2, uploader=self.user
        )

    def test_get_user_album_returns_image_urls_for_user(self):
        response = self.client.get(self.url + "anna/album/", {"username": "anna"})

        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.data, list)

        actual_urls = {item["image"] for item in response.data}
        expected_urls = {self.image1.url.url, self.image2.url.url}
        self.assertEqual(actual_urls, expected_urls)

    def test_get_user_album_user_not_found_returns_404(self):
        url = "/api/users/no-existeix/album/"
